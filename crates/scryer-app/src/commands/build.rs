//! Orchestrating the model build and the drift check — the two agent runs the
//! desktop's toolbar starts.
//!
//! Copied from `src-tauri/src/build.rs` (upstream 0.4.12). The desktop keeps
//! its own copy — this crate is additive and never edits that file — so each
//! rebase diffs the two and carries the delta across.
//!
//! Differences: agent events and the active-node signal go to the
//! [`EventSink`], each naming the PROJECT it came from, instead of into one
//! Tauri window; and the runtime and cancel flag come from the project's own
//! [`AgentState`](super::agent_state::AgentState) rather than one pair shared
//! by the whole app, so a stop in one project never reaches another's session.
...
#![allow(clippy::type_complexity, clippy::unnecessary_sort_by)]
...
use super::preview::{config_for_launch, find_scryer_mcp};

/// One agent event onto the service's stream, named with the project it came
/// from — the desktop's `app.emit("agent-event", …)`, which never had to say
/// which project because a window only ever showed one.
fn agent_event(sink: &Arc<dyn EventSink>, cwd: &str, ev: &scryer_acp::AgentEvent) {
    let payload = serde_json::to_value(ev).unwrap_or_default();
    sink.publish(Event::agent_event(Path::new(cwd), payload));
}

/// Which nodes the build is working on right now, same idea.
fn active_nodes(sink: &Arc<dyn EventSink>, cwd: &str, ids: Vec<String>) {
    sink.publish(Event::build_active_node(Path::new(cwd), ids));
}
...
pub async fn start_model_build(state: &AppState, project_path: &str) -> CommandResult<String> {
    // Registering resolves the path and gives the run the project's OWN agent
    // runtime and cancel flag — a stop here never reaches another project.
    let registered = state.add_project(Path::new(project_path))?;
    let cwd = registered.path().to_string_lossy().to_string();
    let sink = state.sink().clone();
    let project = registered.path();
    ...
    let runtime = registered.agents().runtime();
    ...
    let cancel_flag = registered.agents().cancel_flag();
    registered.agents().arm();

    // 4. Background orchestrator: Wave 1, then a bounded-parallel Wave 2.
    tokio::spawn(async move {
        let emit_msg = |text: String| {
            orch_log(&cwd, &text);
            agent_event(&sink, &cwd, &scryer_acp::AgentEvent::Message { text });
        };
        let emit_fail = |error: String| {
            orch_log(&cwd, &format!("✗ build failed: {error}"));
            agent_event(&sink, &cwd, &scryer_acp::AgentEvent::Failed { error });
        };
        ...
        if cancel_flag.load(std::sync::atomic::Ordering::SeqCst) {
            ...
            agent_event(&sink, &cwd, &scryer_acp::AgentEvent::Cancelled);
            return;
        }
        ...
        // concurrently with every container session.
        {
            ...
            handles.push(tokio::spawn(async move {
                ...
                }
                {
                    ...
                    active_nodes(&sink, &cwd, a.iter().cloned().collect());
                }
...
/// newer changes.
pub async fn start_drift_check(state: &AppState, project_path: &str) -> CommandResult<String> {
    let registered = state.add_project(Path::new(project_path))?;
    let cwd = registered.path().to_string_lossy().to_string();
    let sink = state.sink().clone();
    let project = registered.path();
